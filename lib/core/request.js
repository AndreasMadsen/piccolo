...
Request.prototype.matchPresenter = function () {
  ...
  this.cache.filepath = this.cache.route.path;
...
Request.prototype.handlePresenter = PresenterRequest;
...
Request.prototype.send = function (file) {
  ...
  file.once('ready', function () {
    ...
    var charset = mime.charsets.lookup(file.type);

    // assume charset is UTF-8 if is handled by a compress stream
    if (!charset && /json|text|javascript/.test(file.type)) {
      charset = 'UTF-8';
    }

    // set mime and charset
    res.setHeader('Content-Type', file.type + (charset ? '; charset=' + charset : ''));
