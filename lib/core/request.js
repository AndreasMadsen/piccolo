...
function Request(piccolo, request, response) {
  ...
  // set request id
  if (requestCount === 10000) requestCount = 0;
  ...
  this.domain.run((function () {
    ...
    // direct piccolo request
      ...
      this.log('info', 'got direct piccolo request: ' + this.url.href);
    ...
    // change table request
      ...
      this.log('info', 'got change table request:' + this.url.href);
    ...
    // static file request
      ...
      this.log('info', 'got static file request:' + this.url.href);
    ...
    // emit 404 error, will be handled by developer or default error handler
      ...
      this.emit('error',  utils.httpError(404, new Error('Could not resolve or find ' + this.url.href)));
    }
  }).bind(this));
}
...
Request.prototype.matchPresenter = function () {
  ...
  this.cache.filepath = this.piccolo.require.resolve(this.cache.route.name);
...
Request.prototype.matchStaticFile = function () {
  this.cache.filepath = path.resolve('/', this.url.pathname);
...
Request.prototype.open = function (compress) {
  ...
  var mimetype = this.response.getHeader('Content-Type') || 'application/octet-stream';
  ...
  // log response done
  this.response.once('close', function (had_error) {
    if (had_error) {
      self.log('error', 'Response stream closed - because of error');
    } else {
      self.log('info', 'Response stream closed');
    }
  });

  // do only compress plain text
  if (/xml|json|text|javascript/.test(this.response.getHeader('Content-Type')) === false) {
    ...
    if ('HEAD' == this.request.method || this.response.statusCode === 304) {
      this.log('info', 'Should not send content because of HEAD or cache');
      this.response.end();
      return false;
    }
...
Request.prototype.send = function (file) {
  ...
  file.once('ready', function () {
    ...
    // set mtime and etag headers
    res.setHeader('Last-Modified', file.mtime.toUTCString());
    res.setHeader('ETag', file.etag);

    // set response code
    if (cache !== 'none' &&
        clientMtime >= file.mtime.getTime() &&
...
// Standart logging interface, should be improved
// but for now just overwrite Request.prototype.log.
...
Request.prototype.log = wrap(function (level, message) {
  // do only log info level, in debug mode
  if (level === 'info' && this.piccolo.get('debug') === false) return;
  ...
  if (message instanceof Error) {
    ...
    content = colors.cyan(this.requestID) + ' :: ' +
              colors[colorLevel(level)]( statusCode + ' ' + http.STATUS_CODES[statusCode] ) + ' - ' +
              colors.grey( message.origin ? message.origin.stack : message.stack );
  } else {
    content = colors.cyan(this.requestID) + ' :: ' +
              colors[colorLevel(level)]( message );
  }
...
Request.prototype.handleError = wrap(function () {
  ...
  var existErrorPresenter = false;
  var router = this.piccolo.build.router;

  if (typeof router.error === 'function') {
    // route the error
    this.cache.route = this.piccolo.build.router.error(status, origin);
    this.cache.filepath = this.piccolo.require.resolve(this.cache.route.name);

    if (!!this.cache.filepath) {
      existErrorPresenter = true;
    }
  }
  ...
  // open stream
  var stream = this.stream ? this.open() : false;

  if (existErrorPresenter) {
    // route though error presenter
    this.handlePresenterRequest();
  } else if (stream) {
    // fall back to an simple write
    stream.write(origin.stack);
    stream.end();
  }
