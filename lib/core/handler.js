...
function route(piccolo) {
  ...
  return function(req, res) {
    console.log('uncatched path: ' + req.url);
    res.end();
  };

  /*
  // Get the route module
  var router = require(piccolo.modules.route);
  ...
  return function(req, res, next) {
    ...
    var resource = router.parse(query);

    // async require the change table
    utils.require(resource.path, function (Table) {

      // Execute change table method with given arguments
      var changeTable = new Table(req, res, next);
          changeTable[resource.method].apply(changeTable, resource.args || []);
    });
  };
  */
}
