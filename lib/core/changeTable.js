...
function sendChunk(self, index) {
  ...
  self.internal.res.write(chunk);

  // end response
  if (index === self.internal.content.length) {
    self.internal.res.end();
  }
}
...
function changeTableAbstract(req, res, query, piccolo, modules) {
  this.internal = {
...
changeTableAbstract.prototype.template = function (templatePath) {
  ...
  // Load template file
  var project = this.internal.piccolo.directories.project;
  fs.readFile(path.join(project, templatePath), 'utf8', function (error, content) {
...
changeTableAbstract.prototype.title = function (headline) {
  ...
  this.internal.res.write(headline);
...
changeTableAbstract.prototype.content = function (content, tag, id) {
  ...
  this.internal.res.write(content);
