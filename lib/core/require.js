...
function compileNativeModule(filepath) {
...
// sync: Setup server side require module
exports.setupRequire = function (piccolo) {
  // Save the static directory where none native module will loaded from
  staticDirectory = piccolo.directories.statics;

  common.natives.forEach(function (name) {
    NativeModule[name] = compileNativeModule(common.modules(name));
  });

  Object.keys(piccolo.modules).forEach(function (name) {
    NativeModule[name] = compileNativeModule(piccolo.modules[name]);
  });

  NativeModule.changeTable = compileNativeModule(common.core('changeTable'));
};
  ...
  request(filename, function (error, content) {
    ...
    try {
      ...
      var compiledWrapper = vm.runInThisContext(wrapper, filename);
          compiledWrapper.call({}, handler.done.bind(handler, null), asyncRequire);
    } catch (vmError) {
