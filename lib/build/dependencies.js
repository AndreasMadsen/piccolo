...
function setupCompiler(piccolo, self, name, callback) {

  var options = {
    source: piccolo.get(name),
    cache: path.join(piccolo.get('temp'), name),
    state: path.join(piccolo.get('temp'), name + '.state.json')
  };

  var cache = self.cache[name];
  var build = self.build[name];

  var compiler = self.compiler[name] = leaflet(options, callback);
  ...
  compiler.handle('js', 'string', function (content, next, file) {
    ...
    cache.dependencies['/' + file.path] = list.map(exec);
...
Dependencies.prototype.resolveSource = function (source) {
  ...
  while (match = pattern.exec(source)) {
    if (match[1] === '' && match[4] === undefined) {
      query[match[2] || 'modules'].push(match[3]);
    }
  }
...
Dependencies.prototype.updateCache = function (callback) {
  ...
  readdir(modulePath, function (error, list) {
    ...
    readdir(presenterPath, function (error, list) {
      ...
      // save directory map
      self.build.presenters.dirMap = list.map(function (value) {
        return value.slice(modulePath.length);
      });
...
function deepResovle(self, rootname, searchname, filepath, result) {
  ...
  if (result[rootname].indexOf(filepath) !== -1) {
    return;
  }
  result[rootname].push(filepath);

  // this filepath has already been resolved
  if (build[searchname]) {
    result[rootname].push.apply(result, build[searchname]);
    return;
  }
  ...
  Object.keys(cache).forEach(function (searchname) {
    cache[searchname].forEach(function (filepath) {
      deepResovle(self, rootname, searchname, filepath, result);
    });
  });
}

function appendModule(self, output, seperator, moduleobj) {
...
function handleMap(self, moduleobj, callback) {
  var rootname = moduleobj.rootname;
  var filepath = path.resolve(self.piccolo.get('modules'), './' + moduleobj.filepath);
