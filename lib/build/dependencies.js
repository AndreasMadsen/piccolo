...
function Dependencies(piccolo, callback) {
  ...
  compiler.handle('js', 'string', function (content, next, file) {
    self.map[file.path] = self.resolveSource(content);
...
Dependencies.prototype.resolveModule = function (source, modulename, cb) {
  ...
  var isPath, resolved, searchQuery, result, query, index, i;
  ...
  // set modulename to null if input is an path
  result = {
    name: isPath ? null : modulename
  };
...
Dependencies.prototype.read = function (modulename, cache, output, query) {

  // open or append to stream
  if (!output) {
    output = new flower.relayStream();
    output.write('{');
  } else {
    output.write('}');
  }
