...
exports.require = (function (callback, scope) {
  var cache = {};
  ...
  return function (filename) {
    ...
    fs.readFile(filename, 'utf8', function (err, content) {
      ...
      var argsNames = 'exports';
      var argsArray = [exports];

      Object.keys(scope).forEach(function (name) {
        argsNames += ', ' + name;
        argsArray.push(scope[name]);
      });

      var wrapper = '(function (' + argsNames + ') { var global = undefined; ' + content + '\n});';

      try {
        // compile wrapper using VM
        var compiledWrapper = vm.runInThisContext(wrapper, filename);
            compiledWrapper.apply({}, argsArray);
      } catch (vmError) {
