...
configs.natives.forEach(function (name) {
  NativeModule[name] = compileSyncModule(common.find('modules', name));
});

// Setup the NativeModule cache with server side abstracts
['changeTable'].forEach(function (name) {
  NativeModule[name] = compileSyncModule(common.find('modules', name + '.server'));
});
...
function asyncRequire(piccolo, filename, callback) {
  ...
  if (filename === 'router') {
    NativeModule['router'] = compileSyncModule(piccolo.get('router'));
    callback(null, NativeModule['router']);
    return;
  }
