...
function require(filename, callback) {
  ...
  // Create module cache
  if (piccolo.requireCache === undefined) {
    piccolo.requireCache = requireCache;
    piccolo.requireCallbacks = requireCallbacks;
  }

  if (filename.indexOf('/piccolo/') !== 0) {
    filename = '/piccolo/modules/?name=' + filename;
  }
