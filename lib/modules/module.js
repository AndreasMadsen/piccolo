...
Module._resolveFilename = function(rootname, modulename, parent) {
  ...
  // get cached maps
  var dirMap = dependenciesMap[rootname].dirMap;
  var packageMap = dependenciesMap[rootname].packageMap;
...
function moduleLoader(rootname) {
  return function (request, callback) {
    ...
    if (rootname === 'modules' && NativeModule.exists(request)) {
      if (callback) {
    ...
    if (Module._source[rootname][filename]) {
      if (callback) {
        callback(null, this._initCompile(rootname, filename)); return;
      }
    ...
    readModule(rootname, filename, cache, function (error, map) {
...
Module.prototype.presenter = moduleLoader('presenter');
...
Module.prototype._compile = function(content, filename) {
  ...
  require.presenter = function() {
    return self.presenter.apply(self, arguments);
  };
